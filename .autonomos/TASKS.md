...
- [/] **[V2-API-DOCS-HARDENING]** Resolve Vue plugin, Nuxt provider, Angular accessibility/dynamic-input, React Native accessibility, static-package documentation, and V2 named-import discrepancies. `Priority: 🟠` `Complexity: M` *(Vue plugin item done 2026-08-06: `SolarIconsPlugin` now writes the `--solar-*` vars on `document.body` client-side, Nuxt pattern; docs/README updated; 5 new tests. Nuxt provider item done 2026-08-06: fixed partial-config default loss (defaults merge in runtime plugin + wrapper), warning for ignored styling options with `provider: false`, doc contradiction resolved (`useSolar` global with `provider: true`, scoped with `false`), README/AGENT updated; 2 new module tests. Angular a11y/dynamic-input item done 2026-08-06: `ariaLabel` bound to `aria-label`, `titleAttr`/`alt` render `<title>` (prefers titleAttr), dynamic wrapper + `SolarIcon` directive forward both; `SolarIcon` now forwards `undefined` so clearing inputs resets the icon (weight only when the component declares it); generator regenerated; 9 new tests; angular build green (TS6059 obsolete). RN a11y item done 2026-08-06: `alt` now maps to `accessible` + `accessibilityLabel` on the `<Svg>` (was documented but unimplemented); `IconProps` gains `alt`; 3 new SSR-render tests via `react-dom/server`; RN build green. Remaining: static docs (broken code blocks), named-import guide.)*
