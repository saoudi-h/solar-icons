...
- [ ] **[DOCS-SHADCN-TOOLTIP-CONTRACT]** Modernize the site-owned Tooltip contract against the current shadcn component shape, preserving the existing trigger composition, delay behavior, content animation, accessibility relationship, and icon-explorer consumers. Add only compatibility hooks first, then verify the rendered tooltips before considering any visual change. **Issue:** `ISSUE-UI-FOUNDATION-REFRESH`. `Priority: 🟠` `Complexity: S`
