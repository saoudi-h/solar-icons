...
- [/] **[FIGMA-PLUGIN]** Public Solar Icons Figma browser and inserter. The production build uses Vite, React, TypeScript, Base UI, Fuse, and plain CSS; it embeds all 7,476 SVGs, product fonts, and exact Iconify package marks locally. The complete grid is virtualized and responsive, live-previews stroke-width changes, and the window resizes from all sides and corners. Geometry controls support Figma-like horizontal scrubbing, keyboard increments, and reset. Fuzzy metadata search, custom accessible selects, persistent instant/selection modes, multi-insert, factory reset, and system/light/dark themes are implemented and validated in Figma. Submitted to Figma Community review on 2026-07-30: [plugin 1664759238792120976](https://www.figma.com/community/plugin/1664759238792120976). Documentation changes are prepared on `feat/figma-plugin-community` and must remain unmerged until approval. `Priority: 🟠` `Complexity: L`
