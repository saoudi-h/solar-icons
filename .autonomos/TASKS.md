...
- [x] **[SESSION-PR]** Integrated all session work into one PR branch `integrate/session-work`: rapatriated Node 26, drawer fix, IconCard fix, vue-app TS6, action v2 (supserseded staged branch left local). Verified docs typecheck + vue-app type-check + lint + YAML. `Priority: 🔴` `Complexity: M`

- [x] **[DEPS-PRUNE]** Removed 19 confirmed-unused deps (knip OOM'd on 40k files, used depcheck + manual triage): docs `@base-ui/react` + `react-select`; vue-app faker + tanstack; angular-app router + forms + cdk; test-RN expo-dev-client + typescript; angular pbd + tala-tsconfig + types/jsdom; dotenv ×3 (kept core's); core plugin-react; nuxt devtools; root tsdown + typescript + vite-tsconfig-paths. Documented keeps (CSS-blind, script binaries, importHelpers tslib, ambient types, dynamic resolves, peers). Gates: build 21/21, typecheck, lint, test, inventory green. Branch: `chore/prune-unused-deps` (local). `Priority: 🔵` `Complexity: M` *See: worklogs/2026-09-24-DEPS-PRUNE.md*
...
- [x] **[DOCS-EXPLORER-HERO-FIXES]** Three fixes: (1) hero wheels reuse category icons cyclically so short categories still fill inner 14 + outer 20 (`takeCycled`, same ≥17 gate); (2) deeper light + lighter dark palettes everywhere (explorer light `#5b21b6`+`#c2410c`, dark `#7dd3fc`+`#fdba74`; hero 5-pair deep-vivid and luminous sequences); (3) color-picker hex inputs now mirror external changes immediately with focus-aware typing protection (removed the 10s resync delay + dead `syncInput` prop in both pickers). Verified: typecheck, oxfmt, oxlint clean; regen counts (Faces 14+20). Rendered checkpoint stays the maintainer's. Branch: `feat/docs-per-theme-colors` (local, not pushed). `Priority: 🟠` `Complexity: M` *See: worklogs/2026-09-24-DOCS-EXPLORER-HERO-FIXES.md*

- [x] **[DOCS-HERO-DENSITY-COLOR]** Revised the dark explorer duo to sky/orange (`#38bdf8`+`#fb923c`, luminous complementary on deep backgrounds) and doubled hero wheel density (inner 7→14, outer 10→20 in `generateHeroUtils.ts`, same ≥17 gate so no category drops out; verified Devices 14+20, Faces 14+7, Astronomy 14+18). Gates: typecheck, oxfmt, oxlint, docs production build green (`typecheck:scripts` red on main for unrelated files; zero new errors from this change). Rendered taste/density checkpoint stays the maintainer's. Branch: `feat/docs-per-theme-colors` (local, not pushed). `Priority: 🔵` `Complexity: S` *See: worklogs/2026-09-24-DOCS-HERO-DENSITY-COLOR.md*

- [x] **[DOCS-COLOR-VIBRANCY]** Replaced the safe first-pass palettes with vibrant complementary duos: explorer violet/orange flagship (light `#6d28d9`+`#ea580c`, dark `#8b5cf6`+`#fbbf24`), hero 5-pair vivid-light and neon-dark sequences. SSR HTML confirms the new dark pair live. Rendered taste validation stays the maintainer's checkpoint. Branch: `feat/docs-per-theme-colors` (local, not pushed). `Priority: 🔵` `Complexity: S`

- [x] **[DOCS-COLOR-DIRECTION]** Implemented per-theme color direction (decided: explorer A + hero A). Explorer: `THEME_DEFAULT_COLORS` (light `#1d4ed8`+`#b45309`, dark `#60a5fa`+`#fbbf24`), SSR/first-paint pinned to dark set, post-mount swap only while slots hold known defaults (user picks never overridden); reset restores the current theme's set. Hero: separate 5-pair light/dark sequences over a shared rotation index. Verified: typecheck, oxfmt, oxlint clean; SSR HTML shows deterministic dark values, no leaked DOM props. Rendered light/dark review at widths remains the maintainer's checkpoint (DESIGN.md records the provisional direction). Branch: `feat/docs-per-theme-colors` (local, not pushed). `Priority: 🔵` `Complexity: M` *See: worklogs/2026-09-24-DOCS-COLOR-DIRECTION.md*

- [x] **[DOCS-DEV-CONSOLE-ERRORS]** Fixed 3 of 4 dev-time console errors (verified in rendered SSR HTML + typecheck + lint). `IconCard` now strips `Icon`/`deprecatedAliases` before spreading onto `motion.div` (fixes both DOM-prop warnings; type reflects the runtime record). Hero readouts gate `resolvedTheme` behind mount so SSR and first client render both use the dark variant (fixes hydration mismatch; same mounted pattern as `SiteThemeToggle`). The fumadocs `RootProvider` script-tag warning is NOT fixable locally (next-themes anti-flash inline script; dev-only, zero prod impact). Branch: `fix/docs-dev-console-errors` (local, not pushed). `Priority: 🔵` `Complexity: S` *See: worklogs/2026-09-24-DOCS-DEV-CONSOLE-ERRORS.md*

- [x] **[DEPS-MAJORS]** Staged majors done and green: vitest 5 (+coverage-v8, hoisted nuxt `vi.mock` to top level), jotai 3, zod 4, dotenv 18, unplugin-vue 8, unplugin-solid 3, @vueuse 15, @types/jsdom 30, @nuxt/devtools beta, changesets 3 (+action v1→v2 with renamed inputs), react 19.3, react-native 0.87, tsdown 0.23 (relaxed one static dts style assertion), oxfmt 0.70, next 16.3.6, fumadocs 16.15.14, exact-pin catch-ups. Repo runtime moved to Node 26.10. Final gates: typecheck 39/39, lint 41/41, test 34/34, docs build + angular-app build + export check 12/12 green. Scoped out: TS7 for vue/svelte/nuxt/angular/codemod (TS-API incompatibility), deprecated lucide-vue-next (replacement decision). Branch: `chore/update-dependencies` (local, not pushed). `Priority: 🟠` `Complexity: L` *See: worklogs/2026-09-24-DEPS-MAJORS.md*

- [x] **[DEPS-UPDATE]** Updated workspace dependencies within semver ranges and verified nothing breaks: `typecheck` 39/39, `lint` 41/41, `test` 34/34, docs production build green, `angular-app` build green. Two breakages from the bump fixed along the way (Angular 22.2 needs Node ≥24.15, installed 24.21.0 via fnm; 3 Tailwind canonical-class lint warnings in the drawer fix). Majors (changesets 3, vitest 5, jotai 3, zod 4, dotenv 18, unplugin-*, @vueuse 15, TS7 for vue/svelte/nuxt/angular) intentionally untouched, follow-up proposal pending. Branch: `chore/update-dependencies`. `Priority: 🟠` `Complexity: M` *See: worklogs/2026-09-24-DEPS-UPDATE.md*

- [x] **[DEV-ENV-RELINK]** Repaired fossilized workspace `node_modules` breaking `pnpm dev` in `apps/docs` (and 34 stale `.bin` wrappers repo-wide): `apps/docs/node_modules/.bin/tsx` still pointed at the pruned `tsx@4.23.9` store path while lockfile/store moved to `4.23.13` (same for `oxlint@1.79.0` vs `1.82.0`). A plain `pnpm i` could not fix it because only root had been relinked. Deleted all workspace `node_modules` and reinstalled from the root under Node 24 + pnpm 12.4.1, then rebuilt `@solar-icons/react` (stale `dist` missed the refreshed catalogue icons, causing dev-server 500s). Verified: `predev` green, dev server 200 on `/` and `/icons`, docs `typecheck` 0 errors, `oxlint`/`oxfmt` clean on touched files. `Priority: 🟠` `Complexity: S` *See: worklogs/2026-09-24-DEV-ENV-RELINK.md*

- [x] **[DOCS-ICONS-MOBILE-FILTER-HEADER]** Fixed the mobile filter drawer / site header overlap on `/icons`. The right-edge `vaul` drawer opened at `inset-y-2` under the floating header pill (`z-60`), so the drawer top hid behind the header and the modal overlay (`z-50`) sat below it. The panel now floats below the header (`top-20` = 72px reserved + 8px gap) with overlay `z-[70]` and content `z-[80]`; added an additive `overlayClassName` to the shared Drawer primitive without changing its default contract. Touched files typeclean, `oxfmt` clean; `oxlint`/`tsx` binaries are missing in this environment (pre-existing) and mobile-viewport touch validation remains manual. `Priority: 🟠` `Complexity: S` *See: worklogs/2026-09-24-DOCS-ICONS-MOBILE-FILTER-HEADER.md*
