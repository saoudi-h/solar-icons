...
- [x] **[EXT-FIRST-BATCH]** First batch of extended icons resolving issue #430 blockers: undecorated `plus`/`minus`/`close`/`exclamation`/`question` (+ alias mapping so synonyms resolve) — shipped through the full pipeline with metadata, visually validated in demo apps. ✅ Machinery shipped (commit `e02a02744`): alias exports in react/vue/svelte/solid/react-native style + dynamic barrels via core `buildAliasMap` (schema-validated aliases in `metadata-descriptions.json`), inert with zero aliases (regenerated all 5 packages: zero diff), E2E-verified with a temporary alias. **The SVG art itself must come from the private Figma fork** (hand-written SVGs would be wiped by the next ZIP refresh) — see runbook below. Angular intentionally not wired (directive-based API needs derived directive classes — do it when a real alias icon exists). Suggested aliases: `add`→`plus`, `close`→`x` (all verified collision-free against the 1,246 icon names).
  **Runbook (maintainer):**
  1. Draw the 5 icons × 6 styles in the private Figma fork (24×24 grid, 1.5px strokes, round caps; duotone accent at 50% opacity)
  2. Run fix → rename → export plugins → download ZIP
  3. `pnpm --filter @solar-icons/core refresh:zip /path/to/solar-icons-svgs.zip` → gate fails listing the new icons (expected)
  4. `pnpm --filter @solar-icons/core fix:icons-metadata` → paste drafts into `metadata-descriptions.json`, curate tags/aliases, keep `state: "beta"`
  5. Re-run `refresh:zip` (idempotent) → green → commit svgs + metadata
  6. `pnpm build` (all packages) → eyeball the 5 icons × 6 styles in the demo apps → flip `state` to `stable` in a follow-up commit
  7. Changeset + PR (link issue #430)
