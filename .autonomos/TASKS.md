...
- [x] **[EXT-DIFF-GATE]** New-icon detection + metadata gate: diff the re-extracted `svgs/` against git HEAD inside `refresh-from-figma-zip.sh` (new = added, missing = removed, changed = modified); any new icon without required metadata (description, tags, attribution, date) makes `generate:svgs --offline`/checks fail the build (extend the existing `check-*.ts` exit(1) pattern). Renames surface as delete+add → keep the hand-curated remap workflow (`metadata-descriptions.json`). ✅ Implemented (commit `bb87d4301`): `pnpm check:icons-metadata` (3 invariants: every icon has a curated entry, no orphan entries, new icons declare an explicit `origin`), `pnpm fix:icons-metadata` (drafts entries for manual paste), wired into `refresh-from-figma-zip.sh` after unzip (ZIP kept on failure, rerun is idempotent). Pre-requisite drift fixed: `file-smile-` → `file-smile` (commit `16dd45ebe`). 14 tests, E2E smoke-tested with a fake icon. `Priority: 🟠` `Complexity: M`
...
- [x] **[EXT-FIRST-BATCH]** First batch of extended icons resolving issue #430 blockers: undecorated `plus`/`minus`/`close`/`exclamation`/`question` (+ alias mapping so synonyms resolve) — shipped through the full pipeline with metadata, visually validated in demo apps. ✅ Machinery shipped (commit `e02a02744`): alias exports in react/vue/svelte/react-native style + dynamic barrels via core `buildAliasMap` (schema-validated aliases in `metadata-descriptions.json`), inert with zero aliases (regenerated all 5 packages: zero diff), E2E-verified with a temporary alias. **The SVG art itself must come from the private Figma fork** (hand-written SVGs would be wiped by the next ZIP refresh). Angular intentionally not wired (directive-based API needs derived directive classes — do it when a real alias icon exists). Suggested aliases: `add`→`plus`, `close`→`x` (all verified collision-free against the 1,246 icon names).
  ✅ **First icon shipped (2026-08-14, commit `a5803367b`): `add`** — drawn by the maintainer in the fork, imported via `refresh:zip` (gate passed on first run after two Figma-side root fixes: `file-smile-`→`file-smile` rename in the fork, and duotone accents set to layer opacity `opacity="0.5"` instead of paint opacity). Metadata curated (`origin: extended`, `state: beta`, `addedAt 2026-08-14`, author "Hakim Saoudi"), docs explorer shows it as `isNew` + under the Extended filter. **Remaining: maintainer eyeballs `add` in the demo apps, then flip `state` to `stable`** in `metadata-descriptions.json`. Note: the maintainer chose the icon name `add` directly (no alias needed; the planned `add`→`plus` alias mapping is moot).
  ✅ **Demo-app gap fixed (2026-08-14, commit `91f7d4ca7`):** react-app/solid-app/svelte-app ship committed icon lists generated from core/svgs; they were manual to regenerate (solid-app still showed 1,246 icons after the import; svelte-app still had pre-V2 names like `BellBing`). Generation now runs automatically in `dev`/`build` (`pnpm generate:icons && vite|next …`), react-app gained the missing script. Lists regenerated (1,247 icons, `Add` present).
- [/] **[EXT-DOCS]** Docs & communication. ✅ Explorer part done (2026-08-14): "Extended" toggle in the FilterBar (filters the grid to `origin: 'extended'` icons, UI-only state, reset-aware, empty-state message) + "new" green dot on IconCard — `isNew` computed at generation time in `apps/docs/scripts/generate-descriptions.ts` against a hardcoded 90-day threshold (`NEW_ICON_THRESHOLD_DAYS`, maintainer-adjustable in code, not a user setting). `IconData` in `generated/descriptions.ts` now carries `origin`/`addedAt`/`author`/`state`/`aliases`/`isNew`. E2E-verified with a temporary extended entry (fields flow → `isNew: true` → typecheck green → reverted clean). Remaining: per-icon attribution display in the detail panel, release announcement flow. `Priority: 🔵` `Complexity: M`
