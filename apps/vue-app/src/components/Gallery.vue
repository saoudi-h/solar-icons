  ...
  <div class="space-y-4">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center gap-3">
          <input
            type="color"
            :value="solar.color.value ?? '#f59e0b'"
            @input="solar.setColor(($event.target as HTMLInputElement).value)"
            class="w-10 h-10 rounded-lg cursor-pointer border-0 shrink-0"
          />
          <input
            type="text"
            :value="solar.color.value ?? '#f59e0b'"
      ...
      <div class="space-y-2">
        <label class="text-sm font-medium text-slate-300"
          >Size: <span class="text-amber-400">{{ solar.size.value ?? 32 }}px</span></label
        >
        <input
          type="range"
          min="16"
          max="64"
          :value="Number(solar.size.value) || 32"
          @input="solar.setSize(parseInt(($event.target as HTMLInputElement).value))"
      ...
      <div class="space-y-2" :class="{ 'opacity-30 pointer-events-none': !isLinearLike }">
        ...
        <input
          ...
          @input="solar.setStrokeWidth(parseFloat(($event.target as HTMLInputElement).value))"
          class="w-full accent-amber-500"
        />
      </div>
      <div class="space-y-2">
    ...
    <div v-if="isDuotone" class="pt-6 border-t border-slate-700/50">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div class="space-y-2">
          ...
          <div class="flex items-center gap-3">
            <input
              ...
              @input="solar.setSecondaryColor(($event.target as HTMLInputElement).value)"
        ...
        <div class="space-y-2">
          <label class="text-sm font-medium text-slate-300"
            >Accent Opacity:
            <span class="text-blue-400">{{ solar.secondaryOpacity ?? 0.5 }}</span></label
          >
          <input
            type="range"
            min="0"
            ...
            @input="
              solar.setSecondaryOpacity(parseFloat(($event.target as HTMLInputElement).value))
            "
    ...
    >
      ...
      >
        ...
        <span
          class="text-xs text-slate-500 group-hover:text-slate-300 truncate w-full text-center"
          >{{ name }}</span
        >
      </div>
    </div>
