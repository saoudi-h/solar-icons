...
- **Duotone defaults are per theme (`THEME_DEFAULT_COLORS` in `context.tsx`).** Coupling formula: same color family, primary near-white on dark / near-black on light, secondary colorful but restrained. Light: `#1e3a8a` + `#2563eb` (deep blues). Dark: `#dbeafe` + `#60a5fa`. SSR and first paint always use the dark set (deterministic); `ThemeColorDefaults` swaps in the light set after mount only while both slots still hold known defaults, so any explicit pick (or reset-to-theme) is never overridden. Reset restores the current theme's set. The hero readouts use the same mounted gate for `resolvedTheme`.
