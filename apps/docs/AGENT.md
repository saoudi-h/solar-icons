...
- **Duotone defaults are per theme (`THEME_DEFAULT_COLORS` in `context.tsx`).** Light: `#6d28d9` + `#ea580c` (violet/orange). Dark: `#8b5cf6` + `#fbbf24`. SSR and first paint always use the dark set (deterministic); `ThemeColorDefaults` swaps in the light set after mount only while both slots still hold known defaults, so any explicit pick (or reset-to-theme) is never overridden. Reset restores the current theme's set. The hero readouts use the same mounted gate for `resolvedTheme`.
