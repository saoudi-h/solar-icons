...
1.  **`@solar-icons/react`**: This is the standard entry point. It uses React's Context API (`SolarProvider` and `useSolar`) to manage global styles. These components include the `"use client"` directive for Next.js App Router compatibility.

2.  **`@solar-icons/react/ssr`**: This is a dedicated SSR entry point. The icon components exported from here **do not** use the React Context API and remain as server components (no `"use client"` directive).
...
With Next.js App Router (React Server Components), the standard `@solar-icons/react` imports work directly in server components thanks to the `"use client"` directive. For pure server components without any client-side JavaScript, use the `/ssr` imports.
