---
title: Icon Renames
description: Icons renamed in v2. Old names are no longer exported.
icon: RoundTransferHorizontalIcon
---

V2 removed 37 icon names without keeping deprecated aliases. Code that imports one of the old names will fail to compile.
...
## Current catalogue normalizations

The catalogue later standardized a few family names without removing the old exports immediately. Current package releases keep these old names as deprecated aliases, while the codemod normalizes them when migrating application code:
