...
Same five variables as [React](/docs/v2/frameworks/react#css-variables).

## CSS classes

Every icon renders with class `solar` plus `solar-{name}-{style}` (e.g., `solar-heart-bold`). See the [React page](/docs/v2/frameworks/react#css-classes) for examples.
