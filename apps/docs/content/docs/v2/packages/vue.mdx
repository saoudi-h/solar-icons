...
Returns reactive `Ref`s. Must be called inside a `<SolarProvider>` or after installing `SolarIconsPlugin`.
...
<Callout type="info">
    The variables are written client-side. In server-rendered apps the first paint uses the CSS
    fallbacks and the values apply after hydration. For an exact first paint, wrap your app in a
    global `<SolarProvider>` instead, or set the variables in your global CSS.
</Callout>
...
Same five variables as [React](/docs/v2/packages/react#css-variables).

## CSS classes

Every icon renders with class `solar` plus `solar-{name}-{style}` (e.g., `solar-heart-bold`). See the [React page](/docs/v2/packages/react#css-classes) for examples.
