...
`SolarIcon` accepts `size`, `color`, `strokeWidth`, `weight`, `secondaryColor`, `secondaryOpacity`, `alt`, `ariaLabel`, and `titleAttr`. All forward to the created icon. Clearing an input (setting it to `undefined`) resets the icon to its defaults.
