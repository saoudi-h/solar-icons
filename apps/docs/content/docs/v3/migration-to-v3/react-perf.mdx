...
`@solar-icons/react-perf` was the pre-v3 performance package: one style per component, tree-shakable, no dynamic switching, no provider. In v3, this model becomes the default. Uninstall `react-perf` and install `react`.
