...
| `secondaryColor`   | `--solar-duotone-color`   | Duotone accent color                       |
| `secondaryOpacity` | `--solar-duotone-opacity` | Duotone accent opacity                     |
...
Set `isolated` to bypass the provider entirely.
...
The icon uses hardcoded defaults (`24px`, `currentColor`, `1.5`) and ignores the CSS variable cascade.
