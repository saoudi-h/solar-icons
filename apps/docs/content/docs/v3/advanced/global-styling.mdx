...
| ----------------------- | ----------------------------------------------------- |
| `solar`                 | Every icon                                            |
| `solar-{name}-{style}`  | Static icons in every framework                       |
| `solar-{name}-{weight}` | Dynamic icons; the suffix matches the rendered weight |

**All frameworks follow the same convention.** Static imports pass the style name in the import path (`@solar-icons/react/bold`) and the class carries the suffix (`solar-heart-bold`). Dynamic imports switch styles at runtime via a `weight` prop; the rendered class reflects the active weight, so `weight="Linear"` produces `solar-heart-linear`.
