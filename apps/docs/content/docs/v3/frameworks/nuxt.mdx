...
| `provider`         | `boolean`          | `'true'`   | Inject global SolarProvider wrapping the app         |
| `color`            | `string`           | —         | Default icon color                                   |
| `size`             | `string \| number` | —         | Default icon size                                    |
...
When `provider: true` (default), the module automatically wraps your app in a `<SolarProvider>` with the configured defaults. No need to add `<SolarProvider>` in `app.vue`.

When `provider: false`, you must add `<SolarProvider>` manually:
