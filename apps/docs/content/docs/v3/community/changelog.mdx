...
- One package per framework. V3 merges `@solar-icons/react-perf` into `@solar-icons/react`.
...
- V3 drops category imports (`@solar-icons/react/category`). Import per-icon per-style.
- V3 drops the `/ssr` subpath. Icons work in SSR by default.
- V3 drops the `mirrored` prop.
