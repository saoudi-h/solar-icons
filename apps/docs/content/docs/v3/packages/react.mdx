---
title: React
description: SolarProvider, useSolar, CSS variables, duotone, and stroke width for React 18.3+.
icon: react
---
...
| `solar-{name}` | Specific to the icon (e.g., `solar-heart`).    |
...
`@solar-icons/react` is ESM-only. `require()` does not work. Use `import` syntax or `await import()`. All V3 packages follow this convention.
