<script lang="ts">
    ...
    function getIcon(name: string) {
        return (styleModules[selectedStyle] as any)[name + 'Icon'];
    }
</script>
...
<div class="space-y-4">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        <div class="space-y-2">
            ...
            <div class="flex flex-wrap gap-2">
                {#each STYLES as s}
                    <button class={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${selectedStyle === s ? 'bg-amber-500 text-slate-900' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`} onclick={() => (selectedStyle = s)}>{s}</button>
                {/each}
            </div>
        </div>
        <div class="space-y-2">
    ...
    <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-4">
        {#each filteredIcons as iconName (iconName)}
            ...
            {#if IconComponent}
                <div class="group flex flex-col items-center justify-center gap-2 p-4 bg-slate-800/30 rounded-xl border border-slate-700/30 hover:bg-slate-700/50 hover:border-amber-500/30 transition-all cursor-pointer" title={iconName}>
                    <div class="flex items-center justify-center" style="min-height: 64px;">
                        <IconComponent stroke-width={isLinearLike ? (Number(solar.strokeWidth)) : undefined} />
                    </div>
