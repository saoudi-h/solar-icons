...
export default function Home() {
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-slate-200">
            <div className="mx-auto max-w-7xl space-y-12 px-4 py-8 sm:px-8">
                ...
                <Section
                    number={1}
                    title="Dynamic Icons"
                    desc={`Import from @solar-icons/react/dynamic/name. One component per icon, style via weight prop.`}>
                    ...
                    <p className="mb-4 text-xs text-slate-500">
                        <code className="rounded-sm bg-slate-700 px-1 text-amber-400">
                            {'import { HomeIcon } from "@solar-icons/react/dynamic/home"'}
                        </code>{' '}
                        &rarr;{' '}
                        <code className="rounded-sm bg-slate-700 px-1 text-emerald-400">
                            {'<HomeIcon weight="Bold" size={32} />'}
                        </code>
                    </p>
                    <div className="grid grid-cols-4 gap-4 sm:grid-cols-6 md:grid-cols-8">
                        {DYNAMIC_ICONS.map(({ Icon, name }) => (
                            <div
                                key={name}
                                className="
                              flex flex-col items-center gap-2 rounded-xl border border-slate-700/30
                              bg-slate-800/30 p-3
                            ">
                ...
                <Section
                    number={2}
                    title="Static Icons"
                    desc="Direct per-style imports. No weight prop needed — import exactly what you need.">
                    <p className="mb-4 text-xs text-slate-500">
                        <code className="rounded-sm bg-slate-700 px-1 text-amber-400">
                            {'import { HomeBoldIcon, HomeLinearIcon } from "@solar-icons/react"'}
                        </code>
                    ...
                    <div className="flex flex-wrap gap-4">
                        <div
                            className="
                          flex flex-col items-center gap-1 rounded-xl border border-slate-700/30
                          bg-slate-800/30 p-3
                        ">
                ...
                <Section
                    number={3}
                    title="Icon Gallery (DynamicIcon)"
                    desc="Browse all {ALL_ICONS.length} icons using {'<DynamicIcon weight={...}>'}. Uses SolarProvider + useSolar() for controls.">
                    <SolarProvider
                        color="#f59e0b"
                        size={32}
                        strokeWidth={1.5}
                ...
                <Section
                    ...
                    <div className="mb-4 flex items-center gap-6">
                        ...
                        <div className="space-y-1">
                            ...
                            <input
                                ...
                                max="64"
                                value={cssSize}
                                onChange={e => setCssSize(parseInt(e.target.value))}
                                className="
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            <div
                                className="flex gap-4 rounded-lg bg-slate-900 p-4"
                                style={
                                    {
                                        '--solar-color': cssColor,
                                        '--solar-size': `${cssSize}px`,
                        ...
                        <div className="space-y-2">
                            <code className="block text-xs text-slate-500">
                                Tailwind [--solar-color:...]
                            </code>
                            <div
                                className="
                              flex gap-4 rounded-lg bg-slate-900 p-4
                              [--solar-color:var(--demo-color)] [--solar-size:var(--demo-size)]
                            "
                                style={
                                    {
                                        '--demo-color': cssColor,
                ...
                <Section
                    ...
                    desc="A separate provider with its own controls.">
                    <ProviderDemo />
                </Section>
                ...
                <Section
                    ...
                    <code className="mb-2 block text-xs text-slate-500">
                        {'.solar { color: var(--solar-color, currentColor); }'}
                    </code>
                    <code className="mb-4 block text-xs text-slate-500">
