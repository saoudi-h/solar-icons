            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
                ...
                <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-300">
                        Size: <span className="text-amber-400">{solar.size ?? 32}px</span>
                    </label>
                    <input
                        type="range"
                        min="16"
                ...
                <div
                    className={`space-y-2 ${!isLinearLike(selectedStyle) ? 'opacity-30 pointer-events-none' : ''}`}
                >
                    <label className="text-sm font-medium text-slate-300">
                        Stroke:{' '}
                        <span className="text-amber-400">{solar.strokeWidth ?? 1.5}</span>
                    </label>
                    <input
                        type="range"
                        min="0.5"
                        ...
                        onChange={(e) =>
                            solar.setStrokeWidth(parseFloat(e.target.value))
                        }
                ...
                <div className="space-y-2">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-slate-200 placeholder-slate-500"
                    />
                </div>
            </div>
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-4">
                {filteredIcons.map((name) => {
                    ...
                    return (
                        <div
                            key={name}
                            className="group flex flex-col items-center justify-center gap-2 p-4 bg-slate-800/30 rounded-xl border border-slate-700/30 hover:bg-slate-700/50 hover:border-amber-500/30 transition-all cursor-pointer"
                            title={name}
                        >
                            <div
                            ...
                            <span className="text-xs text-slate-500 group-hover:text-slate-300 truncate w-full text-center transition-colors">
                                {name}
                            </span>
                        </div>
                    )
