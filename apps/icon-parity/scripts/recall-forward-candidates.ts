...
function score(solar: string, lucide: LucideIcon): number {
    ...
    const overlap = solarTokens.filter(solarToken =>
        lucideTokens.some(lucideToken => equivalentToken(solarToken, lucideToken))
    ).length
...
function main(): void {
    if (!fs.existsSync(path.join(atlasRoot, 'lucide/index.json'))) {
        throw new Error(
            'Lucide atlas missing. Run `pnpm --filter icon-parity generate:atlases` first.'
        )
    }
    const lucide = readJson<{ icons: LucideIcon[] }>(
        path.join(atlasRoot, 'lucide/index.json')
    ).icons
    const entries = Array.from(
        { length: 13 },
        (_, index) =>
            readJson<ProductionSheet>(
                path.join(
                    compareRoot,
                    'lucide-production',
                    `sheet-${String(index + 1).padStart(2, '0')}.json`
                )
            ).entries
    ).flat()
    const rows = entries
        .filter(entry => entry.referenceId === null)
        .map(entry => ({
    ...
    fs.writeFileSync(
        output,
        `${JSON.stringify({ version: 1, direction: 'solar-to-lucide', rows }, null, 2)}\n`
    )
