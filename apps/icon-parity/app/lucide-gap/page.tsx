...
function rowMatchesFilter(row: CoverageRow, filter: Filter): boolean {
    ...
    if (filter === 'reverse-match')
        return row.semanticDecision === 'no-match' && row.reverseReview?.decision === 'equivalent'
...
export default function LucideGapPage() {
    ...
    const rows = useMemo(
        () =>
            coverageReport.entries.filter(row => {
                if (!rowMatchesFilter(row, filter)) return false
                if (!deferredQuery) return true
                return `${row.id} ${row.name} ${row.aliases.join(' ')} ${row.reverseReview?.solarCandidates.join(' ') ?? ''}`
                    .toLowerCase()
    ...
    const exactSolarCandidates = selected
        ? [
              ...
              .filter((id, index, all) => all.indexOf(id) === index && !fallbackIds.has(id))
              .map(id => SOLAR_BY_ID.get(id))
              .filter(Boolean)
              .slice(0, 8)
        : []
    const fallbackSolarCandidates = selected
        ? selected.fallbackSolarMatches.map(match => SOLAR_BY_ID.get(match.solarId)).filter(Boolean)
        : []
        ...
        ? [
              ...
              .filter(
                  (id, index, all) =>
                      ...
                      !fallbackIds.has(id) &&
                      !selected.semanticSolarMatches.some(match => match.solarId === id)
              )
              .map(id => SOLAR_BY_ID.get(id))
              .filter(Boolean)
              .slice(0, 8)
                ...
                <header className="flex flex-col gap-5 border-b border-neutral-800 pb-6 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <div className="mt-5 flex flex-wrap items-end gap-3">
                            ...
                            <span className="mb-2 rounded-full border border-rose-300/30 px-2 py-1 text-[10px] font-semibold tracking-[0.14em] text-rose-300 uppercase">
                                Lucide → Solar
                            </span>
                        </div>
                        <p className="mt-3 max-w-3xl text-sm/6 text-neutral-400">
                            Inventaire inverse : séparer les remplacements exacts, les fallbacks
                            contextuels et les vrais gaps Lucide → Solar. Un fallback est utile dans
                            certains contextes, mais ne remplace pas une équivalence précise.
                        </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-3 text-xs text-neutral-400">
                        <span>
                            <strong className="text-rose-300">
                                {formatNumber(
                                    coverageReport.entries.filter(row => row.reverseTier === 'gap')
                                        .length
                                )}
                            </strong>{' '}
                            GAPS
                        ...
                        <span>
                            <strong className="text-orange-300">
                                {formatNumber(
                                    coverageReport.entries.filter(
                                        row => row.reverseTier === 'fallback'
                                    ).length
                                )}
                            </strong>{' '}
                            FALLBACKS
                        ...
                        <span>
                            <strong className="text-amber-300">
                                {formatNumber(
                                    coverageReport.entries.filter(
                                        row =>
                                            row.semanticDecision === 'no-match' &&
                                            row.reverseReview?.decision === 'equivalent'
                                    ).length
                                )}
                            </strong>{' '}
                            REVERSE
                    ...
                    <aside className="min-w-0 rounded-2xl border border-neutral-800 bg-neutral-950/45 p-3 lg:sticky lg:top-5 lg:h-[calc(100vh-40px)]">
                        <div className="px-2 py-1">
                            ...
                            <div className="mt-1 text-xs text-neutral-400">
                                ...
                                {formatNumber(coverageReport.entries.length)}
                            </div>
                        </div>
                        <div className="mt-4 space-y-2">
                            <label className="sr-only" htmlFor="lucide-gap-search">
                                Rechercher
                            </label>
                            <input
                                id="lucide-gap-search"
                                type="search"
                                value={query}
                                onChange={event => setQuery(event.currentTarget.value)}
                                placeholder="Lucide, alias ou ID"
                            ...
                            />
                            <label className="sr-only" htmlFor="lucide-gap-filter">
                                Filtre inverse
                            </label>
                            <select
                                id="lucide-gap-filter"
                                value={filter}
                                ...
                                <option value="gap">GAPS À COMBLER</option>
                                <option value="fallback">FALLBACKS CONTEXTUELS</option>
                                <option value="reverse-match">TROUVÉS EN REVERSE</option>
                                <option value="covered">DÉJÀ COUVERTS</option>
                                <option value="all">TOUS LES ICONES</option>
                            </select>
                        </div>
                        <div className="mt-4 h-[calc(100%-142px)] space-y-0.5 overflow-y-auto pr-1">
                            {rows.map(row => (
                                <button
                                    ...
                                    className={`flex min-h-12 w-full items-center gap-3 rounded-xl px-2.5 text-left hover:bg-neutral-900 ${selected?.id === row.id ? 'bg-amber-300 text-neutral-950' : 'text-neutral-300'}`}>
                                    <span
                                        className={`flex size-8 shrink-0 items-center justify-center rounded-lg ${selected?.id === row.id ? 'bg-neutral-950 text-amber-300' : 'bg-neutral-900 text-neutral-300'}`}>
                                        <LucideGlyph name={row.name} size={23} />
                                    </span>
                                    <span className="min-w-0 flex-1">
                                        ...
                                        <span
                                            className={`block truncate text-[10px] ${selected?.id === row.id ? 'text-neutral-700' : 'text-neutral-500'}`}>
                                            {row.id} · {row.coverage}
                                        </span>
                                    </span>
                                    <span
                                        className={`text-[9px] font-bold ${row.reverseTier === 'exact' ? 'text-emerald-300' : row.reverseTier === 'fallback' ? 'text-orange-300' : row.reverseReview?.decision === 'equivalent' ? 'text-amber-300' : 'text-rose-300'}`}>
                                        {row.reverseTier === 'exact'
                                            ? 'EXACT'
                                            : row.reverseTier === 'fallback'
                                              ? 'FALLBACK'
                                              : row.reverseReview?.decision === 'equivalent'
                                                ? 'REVERSE'
                                                : 'GAP'}
                        ...
                        {selected ? (
                            <>
                                <div className="flex flex-col gap-4 border-b border-neutral-800 pb-5 sm:flex-row sm:items-end sm:justify-between">
                                    <div>
                                        <div className="text-[10px] font-semibold tracking-[0.16em] text-rose-300 uppercase">
                                            {filterLabel(filter)}
                                        </div>
                                ...
                                <div
                                    className={`mt-5 rounded-2xl border p-5 ${selected.reverseTier === 'exact' ? 'border-emerald-900/70 bg-emerald-950/20' : selected.reverseTier === 'fallback' ? 'border-orange-900/70 bg-orange-950/15' : selected.reverseReview?.decision === 'equivalent' ? 'border-amber-900/70 bg-amber-950/15' : 'border-rose-900/60 bg-rose-950/15'}`}>
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                        <span className="text-[10px] font-semibold tracking-[0.16em] text-neutral-400 uppercase">
                                            Lucide → Solar
                                        </span>
                                        <span
                                            className={`rounded-full px-3 py-1 text-xs font-bold ${selected.reverseTier === 'exact' ? 'bg-emerald-400/20 text-emerald-300' : selected.reverseTier === 'fallback' ? 'bg-orange-400/20 text-orange-300' : selected.reverseReview?.decision === 'equivalent' ? 'bg-amber-400/20 text-amber-300' : 'bg-rose-400/20 text-rose-300'}`}>
                                            {selectedLabel}
                                        </span>
                                    </div>
                                    <div className="mt-5 rounded-xl border border-neutral-800 bg-[#f2f0e8] p-5 text-neutral-950">
                                        <div className="text-[10px] font-semibold tracking-[0.16em] text-neutral-500 uppercase">
                                            Lucide · {selected.id}
                                        </div>
                                        <div className="flex min-h-56 items-center justify-center">
                                            <LucideGlyph name={selected.name} size={210} />
                                        </div>
                                    ...
                                    <div className="mt-5">
                                        {exactSolarCandidates.length > 0 && (
                                            <>
                                                <div className="mb-3 text-[10px] font-semibold tracking-[0.16em] text-emerald-300 uppercase">
                                                    Remplacement exact / préféré
                                                </div>
                                                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                                                    {exactSolarCandidates.map(icon => (
                                                        <div
                                                            key={icon?.name}
                                                            className="rounded-xl border border-emerald-900/50 bg-[#f2f0e8] p-3 text-neutral-950">
                                        ...
                                        {fallbackSolarCandidates.length > 0 && (
                                            <>
                                                <div className="mt-5 mb-3 text-[10px] font-semibold tracking-[0.16em] text-orange-300 uppercase">
                                                    Fallback — perte de précision acceptée selon le
                                                    contexte
                                                </div>
                                                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                                                    {fallbackSolarCandidates.map(icon => (
                                                        <div
                                                            key={icon?.name}
                                                            className="rounded-xl border border-orange-900/50 bg-[#f2f0e8] p-3 text-neutral-950">
                                                            <div className="flex min-h-32 items-center justify-center">
                                                                <SolarGlyph
                                                                    icon={icon}
                                                                    style={style}
                                        ...
                                        {evidenceSolarCandidates.length > 0 && (
                                            <>
                                                <div className="mt-5 mb-3 text-[10px] font-semibold tracking-[0.16em] text-neutral-400 uppercase">
                                                    Preuves / candidats non intégrés
                                                </div>
                                                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                                                    {evidenceSolarCandidates.map(icon => (
                                                        <div
                                                            key={icon?.name}
                                                            className="rounded-xl border border-neutral-800 bg-[#f2f0e8] p-3 text-neutral-950">
                                                            <div className="flex min-h-32 items-center justify-center">
                                                                <SolarGlyph
                                                                    icon={icon}
                                                                    style={style}
                                        ...
                                        {exactSolarCandidates.length +
                                            fallbackSolarCandidates.length +
                                            evidenceSolarCandidates.length ===
                                            0 && (
                                            <div className="rounded-xl border border-dashed border-neutral-800 p-6 text-center text-sm text-neutral-500">
                                                Aucun candidat Solar enregistré.
                                            </div>
                                        )}
                                    </div>
                                    {selected.reverseCoverageNote && (
                                        <p className="mt-4 text-xs/5 text-orange-200">
                                            Politique de couverture : {selected.reverseCoverageNote}
                                        </p>
                                    )}
                                    {selected.reverseReview && (
                                        <p className="mt-2 text-xs/5 text-neutral-400">
                                            Revue inverse : {selected.reverseReview.note}
                                        </p>
                                    )}
                                </div>
                            </>
