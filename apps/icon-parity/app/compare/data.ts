...
export interface Candidate {
    ...
    label: 'same name lead' | 'semantic lead' | 'context lead' | 'structural lead'
...
const SHAPE_TOKENS = new Set(['caret', 'chevron'])
const DIRECTIONS = ['up', 'down', 'left', 'right'] as const

function directionFor(icon: SolarIconRecord): string | undefined {
    const tokens = nameTokens(icon.name)
    return DIRECTIONS.find(direction => tokens.includes(direction))
}
...
export function candidatesFor(
    ...
    const direction = directionFor(icon)
    ...
    // Preserve shape-family alternatives when lexical matches would otherwise
    // crowd them out, such as a chevron competing with a named arrow.
    for (const name of collection.names) {
        if (isStructuralCandidate(name, direction)) pool.add(name)
    }
