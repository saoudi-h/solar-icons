        ...
        ">
            ...
            ">
                ...
                <div className="text-right text-xs text-neutral-400">
                    <div>Group {formatNumber(safeGroupIndex + 1)} of {formatNumber(groupCount)}</div>
                    <div className="mt-1">Icons {formatNumber(icons.length ? groupStart + 1 : 0)}-{formatNumber(groupEnd)} of {formatNumber(icons.length)}</div>
                    <div className={reviewedCount === visibleIcons.length ? `
            ...
            ">
                <p className="text-[11px] text-neutral-400">Leads are not verdicts. Each group has a fixed boundary; complete it before moving to the next group.</p>
                <div className="flex shrink-0 gap-2">
                    <button type="button" disabled={safeGroupIndex === 0} onClick={() => setGroupIndex(value => Math.max(value - 1, 0))} className="
                    ...
                    <button type="button" disabled={safeGroupIndex >= groupCount - 1} onClick={() => setGroupIndex(value => Math.min(value + 1, groupCount - 1))} className="
                      min-h-10 rounded-xl border border-neutral-700 px-3 text-xs
                      font-medium text-neutral-200 transition-colors
                      hover:border-amber-300 hover:text-amber-300
                      disabled:cursor-not-allowed disabled:opacity-35
