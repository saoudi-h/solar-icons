...
const REJECTED_BY_KEY = new Map(
    REJECTED_CANDIDATES.map(
        entry => [candidateKey(entry.solar, entry.source, entry.reference), entry] as const
    )
)
const VERIFIED_BY_KEY = new Map(
    VERIFIED_MATCHES.filter(match => match.reference !== null).map(
        match =>
            [candidateKey(match.solar, match.source, match.reference as string), match] as const
    )
)
...
function isFullyReviewed(iconName: string): boolean {
    return SOURCE_DEFINITIONS.every(source => verifiedSlot(iconName, source.id) !== undefined)
}
    ...
    return (
        <button
            ...
            <div className="mt-3 min-w-0">
                ...
                `}>
                    {candidate.label}
                    {displayCategory(candidate.sourceCategory)
                        ? ` · ${displayCategory(candidate.sourceCategory)}`
                        : ''}
                </div>
            </div>
            {candidate.verified && (
              ...
              ${
                  selected
                      ? `bg-neutral-950/10 text-neutral-800`
                      : `
                border border-emerald-900/70 text-emerald-300
              `
              }
            `}>
                    Reviewed: {candidate.verified.decision}
                </div>
            )}
            <div
                className={`
...
function RenderedComparison({
    ...
    icon: SolarIconRecord
    style: SolarStyle
    candidate: CandidateWithSource | undefined
}) {
    return (
        <div
            className="
            ...
            ">
                ...
                ">
                    ...
                    ">
                      ...
                      sm:size-48
                    ">
                            <SolarPreview icon={icon} style={style} />
                        </div>
                    </div>
                ...
                ">
                    <div
                      ...
                      font-semibold tracking-[0.12em] text-neutral-600 uppercase
                    ">
                        <span>{candidate?.source.label ?? 'Reference'}</span>
                        <span>{candidate?.label ?? 'No lead'}</span>
                    </div>
                    <div
                        className="
                      flex flex-1 items-center justify-center py-7
                    ...
                    ">
                                No semantic lead yet
                            </span>
                        )}
                    </div>
                    <code className="truncate text-xs text-neutral-600">
                        {candidate?.sourceName ?? 'Select a candidate below'}
                    </code>
                </div>
            </div>
            <p
              ...
              text-neutral-600
            ">
                The displayed candidate is a lead, not a verdict. Classify the concept and its
                specificity, not just the silhouette.
            </p>
        </div>
    )
}
...
function DecisionButton({
    ...
    const colors = {
        green: active
            ...
            : 'border-emerald-900 text-emerald-300 hover:border-emerald-500',
        blue: active
            ? 'border-sky-300 bg-sky-300 text-neutral-950'
            : 'border-sky-900 text-sky-300 hover:border-sky-500',
        red: active
            ? 'border-rose-300 bg-rose-300 text-neutral-950'
            : 'border-rose-900 text-rose-300 hover:border-rose-500',
    }[tone]
...
function BatchReview({
    ...
    const groupCount = Math.max(Math.ceil(icons.length / groupSize), 1)
    const safeGroupIndex = Math.min(groupIndex, groupCount - 1)
    const groupStart = safeGroupIndex * groupSize
    const visibleIcons = icons.slice(groupStart, groupStart + groupSize)
    const groupEnd = Math.min(groupStart + visibleIcons.length, icons.length)
    const reviewedCount = visibleIcons.filter(icon => isFullyReviewed(icon.name)).length
        ...
        ">
            ...
            ">
                <div>
                    ...
                    ">
                        Compare a group, not a queue
                    </h2>
                    <p className="mt-2 max-w-2xl text-xs/5 text-neutral-400">
                        Each row is a visual review unit. Retrieval leads are intentionally broad;
                        the verdict comes from comparing the rendered concept and its specificity,
                        not from the retrieval signal.
                    </p>
                </div>
                <div className="text-right text-xs text-neutral-400">
                    <div>
                        Group {formatNumber(safeGroupIndex + 1)} of {formatNumber(groupCount)}
                    </div>
                    <div className="mt-1">
                        Icons {formatNumber(icons.length ? groupStart + 1 : 0)}-
                        {formatNumber(groupEnd)} of {formatNumber(icons.length)}
                    </div>
                    <div
                        className={
                            reviewedCount === visibleIcons.length
                                ? `
                      mt-1 text-emerald-300
                    `
                                : `mt-1 text-amber-300`
                        }>
                        {formatNumber(reviewedCount)}/{formatNumber(visibleIcons.length)} complete
                    </div>
                </div>
            </div>
                        ...
                        ">
                            ...
                            ">
                                <span
                                  ...
                                  text-neutral-950
                                ">
                                    <SolarGlyph icon={icon} style={style} size={28} />
                                </span>
                            ...
                            {SOURCE_DEFINITIONS.map(source => {
                                ...
                                const leads = collection
                                    ? candidatesFor(icon, collection, 2).filter(
                                          lead =>
                                              !REJECTED_BY_KEY.has(
                                                  candidateKey(
                                                      icon.name,
                                    ...
                                    <div key={source.id} className="min-w-0 p-2">
                                        ...
                                        ">
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => onOpenIcon(icon)}
                                                title={`${source.label}: ${verified.reference}`}
                                                className="
                                        ...
                                        ">
                                                {leads.map(lead => (
                                                    <button
                                                        ...
                                                        type="button"
                                                        onClick={() => onOpenIcon(icon)}
                                                        title={`${source.label}: ${lead.sourceName}`}
                                                        className="
                                          min-w-0 rounded-lg border
            ...
            <div
                className="
              mt-4 flex flex-col gap-3 border-t border-neutral-800 pt-4
              sm:flex-row sm:items-center sm:justify-between
            ">
                <p className="text-[11px] text-neutral-400">
                    Leads are not verdicts. Each group has a fixed boundary; complete it before
                ...
                <div className="flex shrink-0 gap-2">
                    <button
                        ...
                        onClick={() => setGroupIndex(value => Math.max(value - 1, 0))}
                        className="
                      min-h-10 rounded-xl border border-neutral-700 px-3 text-xs
                    ...
                    <button
                        type="button"
                        disabled={safeGroupIndex >= groupCount - 1}
                        onClick={() => setGroupIndex(value => Math.min(value + 1, groupCount - 1))}
                        className="
                      min-h-10 rounded-xl border border-neutral-700 px-3 text-xs
                      font-medium text-neutral-200 transition-colors
                      hover:border-amber-300 hover:text-amber-300
...
export default function ComparePage() {
    ...
    const [collectionStates, setCollectionStates] =
        useState<Record<SourceId, CollectionState>>(INITIAL_COLLECTION_STATE)
            ...
            loadCollection(source)
                .then(snapshot => {
                    ...
                    setCollectionStates(current => ({
                        ...current,
                        [source.id]: { status: 'ready', snapshot },
                    }))
                })
                .catch(error => {
                    if (!active) return
    ...
    const activeSolar = useMemo(
        () =>
            filteredSolar.find(icon => icon.name === selectedSolarName) ??
            filteredSolar.at(0) ??
            null,
    ...
    const candidateGroups = useMemo(() => {
        ...
        return SOURCE_DEFINITIONS.map(source => {
            ...
            const candidates = verified?.reference
                ? [
                      {
                          sourceName: verified.reference,
                          sourceCategory: collection?.categoryByName[verified.reference],
                          label: 'same name lead' as const,
                          evidence: {
                              nameHits: [],
                              contextHits: [],
                              directName: false,
                ...
                : verified?.decision === 'no-match'
                  ? []
                  : collection
                    ? candidatesFor(activeSolar, collection).filter(
                          candidate =>
                              !REJECTED_BY_KEY.has(
                                  candidateKey(activeSolar.name, source.id, candidate.sourceName)
                              )
                      )
                    : []
            ...
            return {
                ...
                candidates: candidates.map(candidate => ({
                    ...
                    decision:
                        reviews[candidateKey(activeSolar.name, source.id, candidate.sourceName)],
                    verified: VERIFIED_BY_KEY.get(
                        candidateKey(activeSolar.name, source.id, candidate.sourceName)
                    ),
                })),
            }
    ...
    const selectedCandidate =
        candidates.find(candidate => candidate.key === selectedCandidateKey) ?? candidates.at(0)
    const readySources = SOURCE_DEFINITIONS.filter(
        source => collectionStates[source.id].status === 'ready'
    ).length
                ...
                ">
                    <div>
                        ...
                        <p className="mt-3 max-w-2xl text-sm/6 text-neutral-400">
                            Find visual equivalents across five reference sets. Names create a
                            shortlist; only a rendered side-by-side review can establish an
                            equivalence.
                        </p>
                    </div>
                    <div
                    ...
                    ">
                        <div className="flex items-center gap-3">
                            <span>
                                <span className="text-white">{formatNumber(readySources)}</span>/5
                                sources ready
                            </span>
                            <span className="size-1 rounded-full bg-neutral-700" />
                            <span>
                                <span className="text-white">{formatNumber(reviewCount)}</span>{' '}
                                visual decisions
                            </span>
                            <span>
                                <span className="text-white">
                                    {formatNumber(VERIFIED_MATCHES.length)}
                                </span>{' '}
                                verified maps
                            </span>
                            <button
                        ...
                        ">
                            {SOURCE_DEFINITIONS.map(source => (
                                <SourceStatus
                                    key={source.id}
                                    source={source}
                                    state={collectionStates[source.id]}
                                />
                            ))}
                        </div>
                        <div
                ...
                ">
                    ...
                    ">
                        ...
                        ">
                                ...
                                ">
                                    ...
                                    <div className="mt-1 text-xs text-neutral-400">
                                        {formatNumber(filteredSolar.length)} visible of{' '}
                                        {formatNumber(SOLAR_ICONS.length)}
                                    </div>
                                </div>
                            ...
                            <div className="mt-4 space-y-2">
                                <label className="sr-only" htmlFor="solar-search">
                                    Search Solar icons
                                </label>
                                <input
                                    id="solar-search"
                                ...
                                <select
                                  ...
                                  focus:border-amber-300/70
                                ">
                                    {(Object.keys(REVIEW_LABELS) as ReviewFilter[]).map(filter => (
                                        <option key={filter} value={filter}>
                                            {REVIEW_LABELS[filter]}
                                        </option>
                                    ))}
                                </select>
                            </div>
                        ...
                        <section className="min-w-0">
                            {activeSolar ? (
                                ...
                                ">
                                        ...
                                        ">
                                            ...
                                            ">
                                                    {activeSolar.category}
                                                </span>
                                                {activeSolar.tags.slice(0, 5).map(tag => (
                                                    <span
                                                        key={tag}
                                                        className="
                                              rounded-full border
                                    ...
                                    ">
                                            ...
                                            <select
                                                ...
                                                onChange={event =>
                                                    setStyle(
                                                        event.currentTarget.value as SolarStyle
                                                    )
                                                }
                                                className="
                                ...
                                ">
                                        ...
                                        ">
                                                ...
                                                ">
                                                        Retrieval leads only. A lead is not a
                                                        verdict; inspect the concept and specificity
                                                        in the rendered comparison.
                                                    </p>
                                                </div>
                                                <span
                                                    className="
                                              ...
                                              tabular-nums
                                            ">
                                                    {formatNumber(candidates.length)} leads
                                                </span>
                                            </div>
                                    ...
                                    ">
                                        ...
                                        ">
                                            ...
                                            {selectedCandidate ? (
                                                ...
                                                ">
                                                    ...
                                                    ">
                                                            <SourcePreview
                                                                ...
                                                                name={selectedCandidate.sourceName}
                                                                size={34}
                                                            />
                                                        </div>
                                                        <div className="min-w-0">
                                                ...
                                                ">
                                                    ...
                                                    ">
                                                      ...
                                                      text-neutral-200
                                                    ">
                                                                {selectedCandidate.evidence.nameHits.join(
                                                                    ', '
                                                                ) || 'none'}
                                                            </span>
                                                        </div>
                                                    ...
                                                    ">
                                                      ...
                                                      text-neutral-200
                                                    ">
                                                                {selectedCandidate.evidence.contextHits
                                                                    .slice(0, 6)
                                                                    .join(', ') || 'none'}
                                                            </span>
                                                        </div>
                                                ...
                                                ">
                                                  ...
                                                  text-emerald-200/80
                                                ">
                                                                {selectedCandidate.verified.note}
                                                            </p>
                                                        </div>
                                                    )}
                                                    <div
                                                  ...
                                                  text-neutral-400
                                                ">
                                                        This lead explains why the candidate is
                                                        here. The decision must account for the
                                                        icon’s exact concept and specificity.
                                                    </div>
                                                    <div className="mt-4 grid gap-2">
                                                        <DecisionButton
                                                            label="Confirm equivalent"
                                                            tone="green"
                                                            active={
                                                    ...
                                                    <a
                                                        href={`${selectedCandidate.source.iconifyUrl}${encodeURIComponent(selectedCandidate.sourceName)}`}
                                                        target="_blank"
                                                        rel="noreferrer"
                                                        className="
                                            ...
                                            ">
                                                    Select a candidate to inspect it beside the
                                                    Solar artwork.
                                                </div>
                                            )}
                                        </aside>
                            ...
                            ">
                                    Search for a Solar icon to begin.
                                </div>
                            )}
                        </section>
                    </div>
