...
- **Release export verification is mandatory:** package builds and unit tests do not prove published subpaths resolve. Before stable, pack each public package and smoke-test every documented entry point with Node/bundler resolution plus `publint`.
- **Stable documentation must match npm state:** do not label V2 stable or use unpinned install snippets while the V2 packages are prereleases or npm `latest` still points to a pre-V2 release.
