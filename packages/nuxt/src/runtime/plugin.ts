    ...
    if (import.meta.client) {
      const apply = () => {
        ...
        if (ctx.size.value != null) {
          const sz = typeof ctx.size.value === 'number' ? `${ctx.size.value}px` : ctx.size.value
          el.style.setProperty('--solar-size', sz)
        }
        else el.style.removeProperty('--solar-size')

        if (ctx.strokeWidth.value != null) el.style.setProperty('--solar-stroke-width', String(ctx.strokeWidth.value))
        else el.style.removeProperty('--solar-stroke-width')

        if (ctx.secondaryColor.value) el.style.setProperty('--solar-duotone-color', ctx.secondaryColor.value)
        else el.style.removeProperty('--solar-duotone-color')

        if (ctx.secondaryOpacity.value != null) el.style.setProperty('--solar-duotone-opacity', String(ctx.secondaryOpacity.value))
        else el.style.removeProperty('--solar-duotone-opacity')
      }
