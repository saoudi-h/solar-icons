...
const parseIconName = (name: string): ParsedIconName => {
  ...
  if (parts.length !== 3) {
    console.error(chalk.red(`Invalid name format: "${name}". Expected 3 parts separated by "/".`));
    process.exit(1);
  }
  ...
  const iconNameKebab = toKebabCase(iconNameRaw!);
...
const fetchImageUrls = async (api: Figma.Api, fileId: string, ids: string[]): Promise<Record<string, string>> => {
  const chunkSize = 500;
...
const generateMetadata = (metadata: Metadata, parsed: ParsedIconName): void => {
  ...
  // Add the icon name to the category in metadata
...
const verifySvgs = async (svgsPath: string, iconWeights: Record<string, string>, metadata: Metadata, categoryMap: Record<string, { simplifiedOriginal: string; simplifiedKebab: string; tags: string[] }>): Promise<void> => {
  ...
  for (const [categoryKebab, data] of Object.entries(metadata.categories)) {
    ...
    const originalCategoryEntry = Object.values(categoryMap).find(cat => cat.simplifiedKebab === categoryKebab);
    const originalCategory = originalCategoryEntry ? originalCategoryEntry.simplifiedOriginal : toKebabCase(categoryKebab);
...
const generateSvgs = async (): Promise<void> => {
  try {
    ...
    const categoryMap: Record<string, { simplifiedOriginal: string; simplifiedKebab: string; tags: string[] }> = {};
    const simplifiedCategorySet = new Set<string>();

    // Create download promises
    const downloadPromises = ids.map(id => limit(async () => {
      ...
      const { style, mainCategoryOriginal, mainCategoryKebab, tags, iconNameKebab } = parsed;
      ...
      if (!categoryMap[parsed.mainCategoryOriginal]) {
        ...
        if (simplifiedCategorySet.has(mainCategoryKebab)) {
          console.error(
            chalk.red(`Collision detected for simplified category name: "${mainCategoryKebab}"`)
          );
          console.error(
          ...
          categoryMap[parsed.mainCategoryOriginal] = { simplifiedOriginal: mainCategoryOriginal, simplifiedKebab: mainCategoryKebab, tags };
        }
      }
      ...
      const iconPath = createIconPath(SVGS_PATH, mainCategoryOriginal, iconWeight, iconNameKebab);
      ...
      try {
        ...
        console.log(chalk.green(`Saved "${parsed.iconNameKebab}" in "${mainCategoryOriginal}" / "${iconWeight}"`));
        totalIconsProcessed++;
      } catch (error) {
