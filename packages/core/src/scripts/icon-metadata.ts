...
/**
 * Extract genuinely added icon names from porcelain status lines. Renames are
 * intentionally excluded: a filename correction must not be mistaken for a
 * new extended icon that requires an `origin` field.
 */
export const extractAddedIconNamesFromGitStatus = (lines: readonly string[]): Set<string> => {
    const names = new Set<string>()
    for (const line of lines) {
        const status = line.slice(0, 2)
        const payload = line.slice(3).trim()
        if (status === '??' || status.includes('A')) {
            const iconName = extractIconNameFromSvgPath(payload)
            if (iconName) names.add(iconName)
            continue
        }
        if (status.includes('R')) continue
    }
    return names
}
