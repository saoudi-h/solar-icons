...
const main = () => {
    ...
    const gitPaths = [
        ...gitList('git ls-files --others --exclude-standard -- svgs/'),
        ...gitList('git diff --cached --name-only --diff-filter=A -- svgs/'),
    ]
    const newIconNames = extractIconNamesFromGitPaths(gitPaths)
