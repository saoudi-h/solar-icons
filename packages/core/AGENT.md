...
| `src/codegen.ts`                 | Codegen helpers shared by every framework package: `Weight`, `WEIGHTS` (canonical order), `StyleKey`, `StyleComponentsMap<T>`, `WEIGHT_MAP`, `buildDeprecatedAliasMap`, and `applyDuotoneStyle` (`'html'`/`'jsx'` formats). Framework generators must import these from `@solar-icons/core`, not re-declare them.                                                                       |
...
| `src/metadata-descriptions.json` | Hand-curated, committed metadata. `deprecatedAliases` records legacy names, their canonical replacement, and the deprecation reason; generators emit compatibility re-exports from this data.                                                                                                                                                                                           |
