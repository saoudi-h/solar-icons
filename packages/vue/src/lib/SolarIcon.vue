...
import type { BaseSolarProps, IconNode } from './types'
...
const finalMirrored = computed(() => props.mirrored || config.mirrored || DEFAULT_SOLAR.mirrored)
</script>
...
<template>
    <svg
        ...
        <SvgNodeRenderer v-for="(node, index) in iconNodes" :key="index" :node="node" />
